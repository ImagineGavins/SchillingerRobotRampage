...
namespace Schillinger_RobotRampage
{
    static class PathFinder
    {
        ...
        static private List<PathNode> findAdjacentNodes(PathNode currentNode, PathNode endNode)
        {
            ...
            if((X > 0) && (!TileMap.IsWallTile(X - 1, Y)) && (!TileMap.IsWaterTile(X - 1, Y)))
            {
            ...
            if((X < 49) && (!TileMap.IsWallTile(X + 1, Y)) && (!TileMap.IsWaterTile(X + 1, Y)))
            {
                adjacentNodes.Add(new PathNode(currentNode, endNode, new Vector2(X + 1, Y), CostStraight + currentNode.DirectCost));
            }
            else
            ...
            if((Y > 0) && (!TileMap.IsWallTile(X, Y - 1)) && (!TileMap.IsWaterTile(X, Y - 1)))
            {
            ...
            if((Y < 49) && (!TileMap.IsWallTile(X, Y + 1)) && (!TileMap.IsWaterTile(X, Y + 1)))
            {
                adjacentNodes.Add(new PathNode(currentNode, endNode, new Vector2(X, Y + 1), CostStraight + currentNode.DirectCost));
            }
            else
            ...
            if((upLeft) && (!TileMap.IsWallTile(X - 1, Y - 1)) && (!TileMap.IsWaterTile(X - 1, Y - 1)))
            {
                adjacentNodes.Add(new PathNode(currentNode, endNode, new Vector2(X - 1, Y - 1), CostDiagonal + currentNode.DirectCost));
            }
            if ((upRight) && (!TileMap.IsWallTile(X + 1, Y - 1)) && (!TileMap.IsWaterTile(X + 1, Y - 1)))
            {
                adjacentNodes.Add(new PathNode(currentNode, endNode, new Vector2(X + 1, Y - 1), CostDiagonal + currentNode.DirectCost));
            }
            if ((downLeft) && (!TileMap.IsWallTile(X - 1, Y + 1)) && (!TileMap.IsWaterTile(X - 1, Y + 1)))
            {
                adjacentNodes.Add(new PathNode(currentNode, endNode, new Vector2(X - 1, Y + 1), CostDiagonal + currentNode.DirectCost));
            }
            if ((downRight) && (!TileMap.IsWallTile(X + 1, Y + 1)) && (!TileMap.IsWaterTile(X + 1, Y + 1)))
            {
                adjacentNodes.Add(new PathNode(currentNode, endNode, new Vector2(X + 1, Y + 1), CostDiagonal + currentNode.DirectCost));
            }
        ...
        {
            if(TileMap.IsWallTile(endTile) || TileMap.IsWallTile(startTile) || TileMap.IsWaterTile(endTile) || TileMap.IsWaterTile(startTile))
            {
                return null;
            }
