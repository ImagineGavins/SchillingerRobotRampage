...
namespace Schillinger_RobotRampage
{
    class Sprite
    {
        ...
        #region ~DrawingAndAnimationProperties~
        ...
        #region ~CollisionRelatedProperties~
        public Rectangle BoundingBoxRect
        {
            get { return new Rectangle((int)worldLocation.X + BoundingXPadding, (int)worldLocation.Y + BoundingYPadding, FrameWidth - (BoundingXPadding * 2), FrameHeight - (BoundingYPadding * 2)); }
        }
        #endregion

        #region ~CollisionDetectionMethods~
        ...
        public bool IsCircleColliding(Vector2 otherCenter, float otherRadius)
        {
            if((Collidable) && (!Expired))
            {
                if(Vector2.Distance(WorldCenter, otherCenter) < (CollisionRadius + otherRadius)) { return true; }
                else { return false; }
            }
            else { return false; }
        }
