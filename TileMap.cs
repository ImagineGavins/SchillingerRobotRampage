...
namespace Schillinger_RobotRampage
{
    static class TileMap
    {
        ...
        public const int WaterTile = 4;
        public const int WallTileStart = 5;
        public const int WallTileEnd = 8;
        public const int FireTile = 9;
        ...
        static public void Initialize(Texture2D tileTexture)
        {
            ...
            tiles.Add(new Rectangle(128, 32, TileWidth, TileHeight));
        ...
        #region ~InformationAboutMapSquares~
        ...
        #region ~InformationAboutMapTiles~
        ...
        static public bool IsWallTile(int tileX, int tileY)
        {
            ...
            return (tileIndex >= WallTileStart) && (tileIndex <= WallTileEnd);
        }
        ...
        // Fire Tiles -----
        static public bool IsFireTile(int tileX, int tileY)
        {
            int tileIndex = GetTileAtSquare(tileX, tileY);

            if (tileIndex == -1) { return false; }

            return tileIndex == FireTile;
        }

        static public bool IsFireTile(Vector2 square)
        {
            return IsFireTile((int)square.X, (int)square.Y);
        }

        static public bool IsFireTileByPixel(Vector2 pixelLocation)
        {
            return IsFireTile(GetSquareByPixelX((int)pixelLocation.X), GetSquareByPixelY((int)pixelLocation.Y));
        }

        // Water Tile -----
        static public bool IsWaterTile(int tileX, int tileY)
        {
            int tileIndex = GetTileAtSquare(tileX, tileY);

            if (tileIndex == -1) { return false; }

            return tileIndex == WaterTile;
        }

        static public bool IsWaterTile(Vector2 square)
        {
            return IsWaterTile((int)square.X, (int)square.Y);
        }

        static public bool IsWaterTileByPixel(Vector2 pixelLocation)
        {
            return IsWaterTile(GetSquareByPixelX((int)pixelLocation.X), GetSquareByPixelY((int)pixelLocation.Y));
        }
        #endregion
        ...
        static public void Draw(SpriteBatch spriteBatch)
        {
            ...
            int startY = GetSquareByPixelX((int)Camera.Position.Y);
            int endY = GetSquareByPixelX((int)Camera.Position.Y + Camera.ViewPortHeight);
        ...
        static public void GenerateRandomMap()
        {
            ...
            int fireChancePerSquare = 5;
            int waterChancePerSquare = 3;
            ...
            for(int x = 0; x < MapWidth; x++)
            {
                for(int y = 0; y < MapHeight; y++)
                {
                    ...
                    if (rand.Next(0, 100) <= fireChancePerSquare)
                    {
                        mapSquares[x, y] = FireTile;
                    }
                    if (rand.Next(0, 100) <= waterChancePerSquare)
                    {
                        mapSquares[x, y] = WaterTile;
                    }
                }
            }
        }
